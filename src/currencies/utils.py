...
async def on_conflict_do_update_currency(api_key: str, code_name: list) -> dict:
...
async def start_create_or_update_currencies(api_key=API_KEY):
...
async def start_auto_update_db():
